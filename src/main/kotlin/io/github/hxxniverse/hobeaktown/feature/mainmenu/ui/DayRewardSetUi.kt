...
import io.github.hxxniverse.hobeaktown.feature.mainmenu.entity.DayReward
...
class DayRewardSetUi : CustomInventory("일일 보상 설정", 54) {
    ...
    init {
        ...
        for((index, pos) in positions.withIndex()) {
            val reward = DayReward.getReward(index)
        ...
        button(6 to 9, ItemStackBuilder(Material.GREEN_STAINED_GLASS_PANE).setDisplayName("적용").build()) {
            ...
            loggedTransaction {
                for ((index, pos) in positions.withIndex()) {
                    ...
                    DayReward.setReward(index, if(item.type == Material.AIR) null else item)
                }
            }

            player.sendMessage("일일 보상이 성공적으로 적용되었습니다.")
        }
    }
}