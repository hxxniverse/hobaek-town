    ...
    fun open(player: Player) {
        AnvilInventory(
            ...
            onClickResult = { result ->
                ...
                if (player.user.cash < cash) {
                    player.sendErrorMessage("코인가 부족합니다.")
                    return@AnvilInventory listOf(AnvilGUI.ResponseAction.replaceInputText("_"))
                }
                ...
                player.user.cash -= cash
                player.user.cash += withdrawMoney
                player.inventory.addItem(*paperMoney.toTypedArray())
                player.sendInfoMessage("${DecimalFormat("#,###").format(cash)} 코인을 출금하였습니다.")
                if (withdrawMoney > 0) {
                    player.sendInfoMessage("${DecimalFormat("#,###").format(withdrawMoney)} 코인는 다시 입금되었습니다.")
                }
